...
>
> **規範 vs 說明**：本檔與 `docs/PRINCIPLES.md` 是規範（「你必須怎麼做」，衝突時以此為準）；`docs/SPEC.md` 是功能規格書（系統現在做什麼，隨功能同步維護）；`docs/` 其餘文件是說明（informative，「系統實際怎麼運作」），協助理解、不可與規範牴觸。若發現說明文件與規範或程式碼牴觸，以規範為準並回報。完整索引見 `docs/README.md`（文件地圖）。
...
- **案主新需求要主動更新**：當使用者轉述「案主/客戶那邊有新要求或要修改」時，即使還沒開始寫程式，也要先把該需求反映進 `docs/SPEC.md`（在對應功能標 `狀態：⏳ 未開始`），讓規格永遠是最新的對焦點，再進入實作。
- **每個功能都要標狀態，並維護總覽**：每個功能用 `✅ 已完成 / 🚧 進行中 / ⏳ 未開始 / 🗑 已下線` 標記，SPEC 開頭維護一張「功能狀態總覽」表。功能一旦**開始動工 → 做到一半 → 驗收通過**，就要連同程式碼把狀態往前推進；有 MVP 範圍的專案（如 cfans）要標明哪些功能屬於 MVP，避免把後期功能誤當成 MVP 範圍。`✅ 已完成` 只在實際可用且通過驗證後才標，不可灌水。
...
- 規格：`docs/SPEC.md`（功能規格書，單一事實來源；功能改完 / 收到案主新需求要主動更新）。
