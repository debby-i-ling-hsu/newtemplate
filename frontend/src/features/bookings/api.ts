...
export async function listPackages(): Promise<Package[]> {
  const resp = await api.get<Paginated<Package>>("/catalog/packages/");
  return resp.data.results;
}
...
export async function listCompensations(): Promise<Compensation[]> {
  const resp = await api.get<Paginated<Compensation>>("/bookings/compensations/");
  return resp.data.results;
}
